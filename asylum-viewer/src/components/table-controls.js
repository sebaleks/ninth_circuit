            ...
            {COLUMN_GROUPS.map(group => {
              ...
              return (
                <div key={group.key} className="border-b border-border last:border-0">
                  <div className="px-3 py-1.5 bg-th-bg text-[10px] tracking-[0.12em] uppercase text-muted font-semibold">
                    {group.label}
                  </div>
                  {groupCols.map(col => (
                    <label
                      key={col}
                      className="flex items-center gap-2 px-3 py-1.5 hover:bg-row-hover cursor-pointer"
                    >
                      <input
                        type="checkbox"
